        ...
        <div style={containerStyle} className={className}>
            <div style={starContainerStyle}>
                {Array.from({length:maxRating}, (_,i)=>(
                    <Star key={i} isFull={tempRating ? tempRating >= i+1 : rating >= i+1} onRate={()=> handleRating(i+1)}
                          onHoverIn={()=> setTempRating(i+1)} onHoverOut={()=> setTempRating(0)}
                          starColor={starColor} size={size}
                    />
                ))}
            </div>
...
function Star({isFull, onRate, onHoverIn, onHoverOut, starColor, size}){
    const starStyle = {
        height: `${size}px`,
        width: `${size}px`,
        cursor: "pointer",
